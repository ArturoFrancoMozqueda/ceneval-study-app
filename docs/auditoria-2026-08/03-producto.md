...
3. **La auditoría de seguridad fue real y encontró algo real.** `docs/SECURITY_TESTING.md` documenta que `rls_auto_enable()` tenía permisos `EXECUTE` indebidos para `anon` y `authenticated`, que se revocaron y se verificó el resultado, con migración incluida (`20260729173157`). Eso es trabajo de calidad profesional.
...
- **El repositorio no refleja la base de datos:** falta el paquete de la segunda versión retirada, y la asignación `curriculum_order = id - 9` de la migración `20260812175550` es frágil — funciona hoy por coincidencia de IDs, no por diseño. Si en el futuro se corrigen o reordenan clases, ese cálculo se rompe en silencio.
