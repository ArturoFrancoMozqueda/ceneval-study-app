        ...
        <div className="grid gap-4 md:grid-cols-2">
          {ordered.map((session) => {
            const percent = session.totalSteps
              ? Math.round((session.completedSteps / session.totalSteps) * 100)
              : 0;
            return (
              <Link className="[content-visibility:auto] rounded-2xl border border-border bg-white p-5 hover:border-brand/30" href={`/clases/${session.id}`} key={session.id}>
                <div className="flex items-start justify-between gap-3">
                  <span className="font-mono text-sm font-semibold text-success">{session.curriculumCode}</span>
                  <span className="rounded-full bg-success-soft px-3 py-1 text-xs font-semibold text-success">Publicada</span>
                </div>
                <h3 className="mt-3 text-lg font-semibold">{session.title}</h3>
                <p className="mt-1 text-sm text-muted">{session.subjectName}</p>
                <p className="mt-3 text-sm text-muted">
                  {session.audioSources.length
                    ? session.audioSources.map((source) => `Audio ${String(source.audioNumber).padStart(2, "0")}${source.fragment ? ` · ${source.fragment}` : ""}`).join(" / ")
                    : "Sin audio asignado"}
                </p>
                <div className="mt-4 h-2 overflow-hidden rounded-full bg-background" aria-label={`Progreso ${percent}%`}>
