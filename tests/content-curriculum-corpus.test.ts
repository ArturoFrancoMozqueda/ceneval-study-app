...
test("el catálogo vigente contiene C01–C57 una sola vez y solo contratos 1.2", async () => {
  const fileNames = (await readdir(packagesDirectory))
    .filter((fileName) => fileName.endsWith(".json"))
    .sort();

  assert.equal(fileNames.length, 57);
  ...
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C47")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C48")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C49")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C50")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C51")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C52")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C53")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C54")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C55")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C56")
      ?.packageVersion,
    "1.2",
  );
  assert.equal(
    currentPackages.find((packageData) => packageData.curriculum.code === "C57")
      ?.packageVersion,
    "1.2",
  );
